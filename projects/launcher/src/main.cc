...
static constexpr wchar_t DLL_NAME[] = L"tc.dll";
...

    {
        ...
        */
        ...
        const std::wstring searchPaths[] = {
            ...
            #if TC_DEBUG
                fs::current_path().parent_path() / L"tc\\build\\bin\\debug",
                fs::current_path().parent_path().parent_path().parent_path().parent_path() / L"tc\\build\\bin\\debug", // This is stupid but it works if you run the launcher from its directory
            #endif
        ...
        if (dllPathString.empty()) {
            ...
            util::displayError(L"Could not locate tc.dll, searched in locations:\n\n{}", joinedPaths);
            return 1;
        }
    }
