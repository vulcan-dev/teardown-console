#ifndef TC_SHARED_UTIL_H
#define TC_SHARED_UTIL_H
...
static void displayMessage(uint32_t type, std::wstring formatted) {
    ...
    MessageBoxW(NULL, formatted.c_str(), L"TC Launcher", type);
...
static void displayMessage(uint32_t type, const std::wstring& format, Args&&... args) {
    ...
    MessageBoxW(NULL, formatted.data(), L"TC Launcher", type);
